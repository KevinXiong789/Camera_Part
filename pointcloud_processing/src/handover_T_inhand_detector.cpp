...
//**********subscribe point cloud from cam1, get right hand position from cam2 + transform matrix between cam1 and cam2*****************can run,but not good
//*********computing max_distance between pointcloud and elbow********************************
/*
#include <rclcpp/rclcpp.hpp>
#include <sensor_msgs/msg/point_cloud2.hpp>
#include <std_msgs/msg/float32_multi_array.hpp>
...
private:
  ...
  void processPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    pcl::PointXYZRGB known_point; // the known point
    known_point.x = right_hand_x;
    known_point.y = right_hand_y;
    known_point.z = right_hand_z;
    float radius = 0.2; // the fixed radius
    std::vector<int> indices;
    std::vector<float> distances;
    pcl::KdTreeFLANN<pcl::PointXYZRGB> kdtree;
    kdtree.setInputCloud(processed_cloud);
    kdtree.radiusSearch(known_point, radius, indices, distances);

    pcl::PointXYZRGB other_known_point; // the other known point
    other_known_point.x = right_elbow_x;
    other_known_point.y = right_elbow_y;
    other_known_point.z = right_elbow_z;
    float max_distance = 0.0;
    for (int i = 0; i < indices.size(); ++i)
    {
        pcl::PointXYZRGB point = processed_cloud->points[indices[i]];
        float distance = sqrt(pow(point.x - other_known_point.x, 2) +
                                pow(point.y - other_known_point.y, 2) +
                                pow(point.z - other_known_point.z, 2));
        if (distance > max_distance)
        {
          max_distance = distance;
        }
    }
    RCLCPP_INFO(this->get_logger(), "Max_distance: %f", max_distance);
...
//*********subscribe point cloud from cam1, get right hand position from cam2 + transform matrix between cam1 and cam2*****************testing
//*********crop, vg sor filters, then get point cloud, get tool grasping position (xyz and angle), then pub*****************************
