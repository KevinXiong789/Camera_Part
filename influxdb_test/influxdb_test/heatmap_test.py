...
'''
# ****************heat map of a moving point************************ can run
import cv2
import numpy as np
...
# Define the position of the moving point
moving_point = (320, 240)  # example position, replace with your actual position
...
'''
# **************nuitrack right hand position********************* can run
...
'''
# ************nuitrack+heatmap every 200 frame (with 'frame = cv2.resize(frame, heatmap_size_new)' version)************** can run
from PyNuitrack import py_nuitrack
import cv2
import numpy as np
...
# Create a VideoCapture object for the D435 camera
heatmap_size_old = (640, 480)
heatmap_size_new = (1440, 1080)
#heatmap_size_new = (1920, 1080)
...
while True:
	...
	#frame = img_depth
	frame = img_color
	frame = cv2.resize(frame, heatmap_size_new)

	cv2.imshow('Video', frame)
	
	# Check if we have enough points to draw the heatmap
	if len(point_list) >= 200:
		# Create the heatmap from the point list
		heatmap_points = np.zeros(heatmap_size_new, dtype=np.float32)
		for point in point_list:
			#x, y = point
			x = point[0] * ratio[0]
			y = point[1] * ratio[1]
			heatmap_points[int(x), int(y)] += 1
		heatmap_points = cv2.GaussianBlur(heatmap_points, (25, 25), 0)
		heatmap_points = cv2.normalize(heatmap_points, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
		heatmap_color = cv2.applyColorMap(np.uint8(heatmap_points * 10), cv2.COLORMAP_JET)
